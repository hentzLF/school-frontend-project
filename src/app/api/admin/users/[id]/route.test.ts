...
describe("admin/users/[id] route", () => {
  ...
  describe("PUT", () => {
    it("should update a user with valid body and return updated user", async () => {
    ...
    it("should call the correct backend endpoint with the user id", async () => {
      mockBackendFetch.mockResolvedValue({ data: mockUser, status: 200 });
    ...
    it("should propagate a backend error response", async () => {
      ...
      const response = await PUT(
        new NextRequest("http://localhost/api/admin/users/u999", {
          method: "PUT",
          body: JSON.stringify({ role: "Provider" }),
        }),
